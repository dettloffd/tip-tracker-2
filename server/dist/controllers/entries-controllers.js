...
const createEntry = async (req, res, next) => {
    ...
    let newEntry;
    try {
        const entryResult = await pool.query(`INSERT INTO entries (date, num_transactions, tips_total, user_id)
         VALUES ($1, $2, $3, $4)
         RETURNING *`, [date, numTransactions, tipsTotal, creator]);
        newEntry = entryResult.rows[0];
    }
    catch (err) {
        console.log(err);
...
const editEntry = async (req, res, next) => {
    ...
    let editedEntry;
    try {
        const editEntryResult = await pool.query(`UPDATE entries SET date = $1, num_transactions = $2, tips_total = $3, updated_at = CURRENT_TIMESTAMP WHERE entry_id = $4 RETURNING *`, [date, numTransactions, tipsTotal, entryId]);
        editedEntry = editEntryResult.rows[0];
    }
    catch (err) {
        console.log(err);
        res.status(500).json({
...
//# sourceMappingURL=entries-controllers.js.map